...
static void* _retrieve(BST_TREE* tree,
        void* dataPtr, 
        NODE* root);
static void _retrieveAll(BST_TREE* tree,
        NODE* root, 
        void* dataPtr, 
...
static void _BFtraverse(NODE* root, void (*process)(void* dataPtr));
...
/*	===================== BST_RetrieveAll ===================== 
    Searches tree for all nodes containing requested key 
    and returns a flag that shows whether data was found or not.
    Pre     takes in the tree, dataPtr to the data to be searched 
            and a pointer to a function to be processed.
    Post    tree searched; flag returned
    Return  flag returned with 1 or 0. 
    */

int BST_RetrieveAll(BST_TREE* tree, void* dataPtr, void (*process)(void* dataPtr))
{
    int flag = 0;
    NODE* found;

    if((found = _retrieve(tree, dataPtr, tree->root)))
    {
        flag = 1;
        _retrieveAll(tree, found, dataPtr, process);
    }
...
/*	===================== _retrieveAll ===================== 
    Searches tree for all nodes containing requested key 
    Pre     _retrieveAll passes tree, root, dataPtr which is
            a struct that contains the key to be searched for
            and a pointer to a function.
    Post    tree searched;  
    Return  does not return anything.
    */

static void _retrieveAll(BST_TREE* tree, NODE* root, void* dataPtr, 
        void (*process)(void* dataPtr))
{
    NODE* found = root;
    
    while (found){
        if(tree->compare(found->dataPtr, dataPtr) == 0)
            process(found->dataPtr); 
        found = _retrieve(tree, dataPtr, found->right);
    }
}
/*	=================== BST_Traverse =================== 
...
/*	=================== BF_Traversal =================== 
    ...
    Pre   Tree has been created (may be null) 
    process ÒvisitsÓ nodes during traversal 
    Post  Nodes processed in breadth first sequence. 
    */

void BFTraversal(BST_TREE* tree, void (*process)(void *dataPtr))
{
